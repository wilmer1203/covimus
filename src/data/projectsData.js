export const projects = [
  {
    id: 1,
    name: "Mejoramiento del Espigón",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Paseo La Cruz y El Mar",
    description: "Mejoramiento de la superficie con material granular.",
    fullDescription: "Como parte del plan integral de mantenimiento de infraestructuras urbanas, la Corporación de Vialidad e Infraestructura del municipio Sotillo (Covimus) ejecuta trabajo de mejoramiento y estabilización del terreno del espigón del emblemático Paseo de la Cruz y el Mar. La intervención técnica consiste en el mejoramiento de la superficie con material granular (ripio) en aproximadamente 200 metros, y su posterior compactación con maquinaria pesada, garantizando una base sólida y duradera para el tránsito en esta zona estratégica de la costa. Con estas acciones, el Alcalde de Sotillo @jesusmarcanot reafirma el compromiso de la recuperación de los espacios públicos y la optimización de la infraestructura vial de la ciudad.",
    images: [
      "/assets/images/ima_projects/2026/Mejoramiento_Espigon/espigon1.webp",
      "/assets/images/ima_projects/2026/Mejoramiento_Espigon/espigon2.webp",
      "/assets/images/ima_projects/2026/Mejoramiento_Espigon/espigon3.webp",
      "/assets/images/ima_projects/2026/Mejoramiento_Espigon/espigon4.webp"
    ],
    image: "/assets/images/ima_projects/2026/Mejoramiento_Espigon/espigon1.webp",
    imageAlt: "Mejoramiento del Espigón",
    status: "completed",
    progress: 100,
    startDate: "02/02/2026",
    endDate: "02/03/2026",
    budget: "Ripio (200m aprox)",
    beneficiaries: "200 Metros",
    contractor: "Covimus",
    coordinates: { lat: 10.220913, lng: -64.641596 },
    impact: "Recuperación de espacios públicos y optimización vial.",
    testimonial: "El cambio es notable. Antes caminar por aquí era difícil por el terreno irregular, ahora se siente mucho más firme y seguro para venir con la familia.",
    testimonialAuthor: "Luisa Rodríguez",
    testimonialRole: "Vecina de Paseo La Cruz y El Mar"
  },
  {
    id: 2,
    name: "Estacionamiento Paseo La Cruz y El Mar",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Paseo La Cruz y El Mar",
    description: "Recuperación integral, demarcación y asfaltado del estacionamiento.",
    fullDescription: "En el marco de la recuperación de espacios públicos liderada por el Alcalde Jesús Marcano Tabata, se ejecutó la rehabilitación integral del estacionamiento del Paseo de la Cruz y el Mar. La obra comprendió la colocación de aproximadamente 350 toneladas de asfalto y la demarcación vial de 3,000 m², garantizando el ordenamiento y la seguridad en este punto clave para el esparcimiento familiar. Esta intervención reafirma el compromiso de la gestión municipal, con el apoyo del gobernador Luis Marcano y el presidente Nicolás Maduro, de elevar la calidad de vida de los ciudadanos y potenciar la infraestructura turística de Puerto La Cruz.",
    images: [
      "/assets/images/ima_projects/2026/Estacionamiento_Paseo_la_Cruz_y_el_Mar/estacionamiento-paseo-01.jpg",
      "/assets/images/ima_projects/2026/Estacionamiento_Paseo_la_Cruz_y_el_Mar/estacionamiento-paseo-02.jpg",
      "/assets/images/ima_projects/2026/Estacionamiento_Paseo_la_Cruz_y_el_Mar/estacionamiento-paseo-03.jpg",
      "/assets/images/ima_projects/2026/Estacionamiento_Paseo_la_Cruz_y_el_Mar/estacionamiento-paseo-04.jpg",
      "/assets/images/ima_projects/2026/Estacionamiento_Paseo_la_Cruz_y_el_Mar/estacionamiento-paseo-05.jpg",
      "/assets/images/ima_projects/2026/Estacionamiento_Paseo_la_Cruz_y_el_Mar/estacionamiento-paseo-06.jpg",
      "/assets/images/ima_projects/2026/Estacionamiento_Paseo_la_Cruz_y_el_Mar/estacionamiento-paseo-07.jpg"
    ],
    image: "/assets/images/ima_projects/2026/Estacionamiento_Paseo_la_Cruz_y_el_Mar/estacionamiento-paseo-01.jpg",
    imageAlt: "Vista aérea del estacionamiento rehabilitado y demarcado",
    status: "completed",
    progress: 100,
    startDate: "01/12/2025",
    endDate: "22/12/2025",
    budget: "390 Ton Asfalto",
    beneficiaries: "2,000 m²",
    contractor: "Covimus",
    coordinates: { lat: 10.227253, lng: -64.634818 },
    impact: "Organización vial y mejora turística del Paseo La Cruz y El Mar.",
    testimonial: "Quedó espectacular el estacionamiento. Hacía falta este orden para quienes visitamos el Paseo los fines de semana. ¡Excelente trabajo!",
    testimonialAuthor: "José Miguel Silva",
    testimonialRole: "Visitante Frecuente"
  },
  {
    id: 3,
    name: "Rehabilitación de la Av. Municipal (Paseo Miranda)",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Paseo Miranda",
    description: "Colocación de 66 toneladas de asfalto en el Casco Central.",
    fullDescription: "Como parte del plan de recuperación vial del Casco Central, la Covimus, en articulación con la Comuna Bahía de Pozuelo, ejecutó la rehabilitación de la Avenida Municipal, específicamente frente al Elevado del Paseo Miranda (Eje 5). Los trabajos consistieron en la colocación de 66 toneladas de mezcla asfáltica en caliente, logrando la recuperación de 99 metros lineales de vialidad. Esta obra optimiza significativamente el flujo vehicular en una de las arterias más transitadas de Puerto La Cruz, garantizando calles más seguras y mejorando la calidad de vida de conductores y transeúntes, gracias a las políticas de inversión del presidente Nicolás Maduro y el gobernador Luis Marcano.",
    images: [
      "/assets/images/ima_projects/2026/Paseo_Miranda/av-municipal-miranda-01.jpg",
      "/assets/images/ima_projects/2026/Paseo_Miranda/av-municipal-miranda-02.jpg",
      "/assets/images/ima_projects/2026/Paseo_Miranda/av-municipal-miranda-03.jpg",
      "/assets/images/ima_projects/2026/Paseo_Miranda/av-municipal-miranda-04.jpg",
      "/assets/images/ima_projects/2026/Paseo_Miranda/av-municipal-miranda-05.jpg",
      "/assets/images/ima_projects/2026/Paseo_Miranda/av-municipal-miranda-06.jpg"
    ],
    image: "/assets/images/ima_projects/2026/Paseo_Miranda/av-municipal-miranda-01.jpg",
    imageAlt: "Asfaltado en Av. Municipal frente al Elevado",
    status: "completed",
    progress: 100,
    startDate: "07/01/2026",
    endDate: "12/01/2026",
    budget: "66 Ton Asfalto",
    beneficiaries: "99 Metros lineales",
    contractor: "Covimus",
    coordinates: { lat: 10.210194, lng: -64.633796 },
    impact: "Optimización del tránsito en el Casco Central.",
    testimonial: null
  },
  {
    id: 4,
    name: "Rehabilitación Av. Municipal (Cristina Suite)",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Av. Municipal",
    description: "Colocación de 260 toneladas de asfalto frente al Cristina Suite.",
    fullDescription: "Continuando con la transformación de Puerto La Cruz, la Alcaldía del municipio Sotillo, en trabajo articulado con la Corporación de Vialidad Municipal (Covimus), colocó 260 toneladas de mezcla asfáltica en caliente en la Avenida Municipal. Los trabajos se concentraron específicamente en el tramo frente al Hotel Cristina Suite y la Torre Porteña, optimizando la carpeta de rodamiento en esta zona de alto tráfico. El alcalde Jesús Marcano Tabata supervisó personalmente la jornada, ratificando su compromiso con la modernización vial y la seguridad de todos los porteños para construir un municipio más transitable.",
    images: [
      "/assets/images/ima_projects/2026/Cristina_Suite/av-municipal-cristina-suite-01.jpg",
      "/assets/images/ima_projects/2026/Cristina_Suite/av-municipal-cristina-suite-02.jpg",
      "/assets/images/ima_projects/2026/Cristina_Suite/av-municipal-cristina-suite-03.jpg",
      "/assets/images/ima_projects/2026/Cristina_Suite/av-municipal-cristina-suite-04.jpg",
      "/assets/images/ima_projects/2026/Cristina_Suite/av-municipal-cristina-suite-05.jpg",
      "/assets/images/ima_projects/2026/Cristina_Suite/av-municipal-cristina-suite-06.jpg"
    ],
    image: "/assets/images/ima_projects/2026/Cristina_Suite/av-municipal-cristina-suite-01.jpg",
    imageAlt: "Asfaltado frente al Cristina Suite y Torre Porteña",
    status: "completed",
    progress: 100,
    startDate: "04/01/2026",
    endDate: "08/01/2026",
    budget: "260 Ton Asfalto",
    beneficiaries: "Tramo Cristina Suite",
    contractor: "Covimus",
    coordinates: { lat: 10.219646, lng: -64.631615 },
    impact: "Modernización y seguridad vial en zona de alto tráfico.",
    testimonial: null
  },
  {
    id: 5,
    name: "Rehabilitación Av. Américo Vespucio (Doral Beach)",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Doral Beach",
    description: "Colocación de 200 toneladas de asfalto frente a Doral Beach.",
    fullDescription: "Iniciando el plan de obras 2026, la Covimus ejecutó la rehabilitación vial de la Avenida Américo Vespucio, específicamente frente al conjunto residencial Doral Beach en el sector La Marina. Esta primera jornada del año, articulada con la Comuna Socialista El Paraíso y Covimus, contempló la colocación de 200 toneladas de mezcla asfáltica en caliente para recuperar 160 metros lineales de vía. Esta obra, respaldada por el gobernador Luis Marcano, mejora sustancialmente la transitabilidad en esta importante zona turística y residencial, garantizando vías dignas y seguras para el pueblo.",
    images: [
      "/assets/images/ima_projects/2026/Doral_Beach/rehabilitacion-americo-vespucio-01.jpg",
      "/assets/images/ima_projects/2026/Doral_Beach/rehabilitacion-americo-vespucio-02.jpg",
      "/assets/images/ima_projects/2026/Doral_Beach/rehabilitacion-americo-vespucio-03.jpg",
      "/assets/images/ima_projects/2026/Doral_Beach/rehabilitacion-americo-vespucio-04.jpg",
      "/assets/images/ima_projects/2026/Doral_Beach/rehabilitacion-americo-vespucio-05.jpg",
      "/assets/images/ima_projects/2026/Doral_Beach/rehabilitacion-americo-vespucio-06.jpg",
      "/assets/images/ima_projects/2026/Doral_Beach/rehabilitacion-americo-vespucio-07.jpg",
      "/assets/images/ima_projects/2026/Doral_Beach/rehabilitacion-americo-vespucio-08.jpg"
    ],
    image: "/assets/images/ima_projects/2026/Doral_Beach/rehabilitacion-americo-vespucio-01.jpg",
    imageAlt: "Asfaltado en Av. Américo Vespucio frente a Doral Beach",
    status: "completed",
    progress: 100,
    startDate: "05/01/2026",
    endDate: "07/01/2026",
    budget: "200 Ton Asfalto",
    beneficiaries: "160 Metros lineales",
    contractor: "Covimus",
    coordinates: { lat: 10.201146, lng: -64.670631 },
    impact: "Recuperación vial en zona turística y residencial.",
    testimonial: null
  },
  {
    id: 6,
    name: "Rehabilitación Calle Los Tubos (Las Delicias)",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Las Delicias",
    description: "Colocación de 200 toneladas de asfalto en el sector Las Delicias.",
    fullDescription: "En el marco del plan de recuperación integral impulsado por el alcalde Jesús Marcano Tabata, se ejecutó la rehabilitación de la Calle Los Tubos en el sector Las Delicias. Con la colocación de 200 toneladas de mezcla asfáltica en caliente, se consolida una vialidad digna y segura para el servicio del pueblo. Este esfuerzo, articulado con el Poder Popular y respaldado por el gobernador Luis Marcano, transforma realidades y dignifica cada rincón del municipio Juan Antonio Sotillo.",
    images: [
      "/assets/images/ima_projects/2025/Los_Tubos_del_sector_Las_Delicias/calle-los-tubos-las-delicias-01.jpg",
      "/assets/images/ima_projects/2025/Los_Tubos_del_sector_Las_Delicias/calle-los-tubos-las-delicias-02.jpg",
      "/assets/images/ima_projects/2025/Los_Tubos_del_sector_Las_Delicias/calle-los-tubos-las-delicias-03.jpg",
      "/assets/images/ima_projects/2025/Los_Tubos_del_sector_Las_Delicias/calle-los-tubos-las-delicias-04.jpg",
      "/assets/images/ima_projects/2025/Los_Tubos_del_sector_Las_Delicias/calle-los-tubos-las-delicias-05.jpg",
      "/assets/images/ima_projects/2025/Los_Tubos_del_sector_Las_Delicias/calle-los-tubos-las-delicias-06.jpg"
    ],
    image: "/assets/images/ima_projects/2025/Los_Tubos_del_sector_Las_Delicias/calle-los-tubos-las-delicias-01.jpg",
    imageAlt: "Asfaltado en Calle Los Tubos, sector Las Delicias",
    status: "completed",
    progress: 100,
    startDate: "28/11/2025",
    endDate: "03/12/2025",
    budget: "110 Ton Asfalto",
    beneficiaries: "510 Metros lineales",
    contractor: "Covimus",
    coordinates: { lat: 10.201325, lng: -64.615322 },
    impact: "Consolidación de vialidad digna y segura para la comunidad.",
    testimonial: null
  },
  {
    id: 7,
    name: "Plan de Asfaltado Av. Municipal",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Av. Municipal",
    description: "Colocación de 200 toneladas de asfalto para optimizar la movilidad.",
    fullDescription: "Dando continuidad al Plan de Asfaltado Integral en el municipio Sotillo, la Alcaldía de Puerto La Cruz a través de Covimus, colocó 200 toneladas de mezcla asfáltica en caliente en un tramo estratégico de la Avenida Municipal. Esta obra optimiza la movilidad urbana y garantiza la seguridad vial, cumpliendo con el compromiso del gobernador Luis Marcano y el alcalde Jesús Marcano Tabata de recuperar los espacios públicos y fortalecer la red vial para el bienestar de los portocruzanos.",
    images: [
      "/assets/images/ima_projects/2025/Av.Municipal/av-municipal-nov2025-01.jpg",
      "/assets/images/ima_projects/2025/Av.Municipal/av-municipal-nov2025-02.jpg",
      "/assets/images/ima_projects/2025/Av.Municipal/av-municipal-nov2025-03.jpg",
      "/assets/images/ima_projects/2025/Av.Municipal/av-municipal-nov2025-04.jpg",
      "/assets/images/ima_projects/2025/Av.Municipal/av-municipal-nov2025-05.jpg"
    ],
    image: "/assets/images/ima_projects/2025/Av.Municipal/av-municipal-nov2025-01.jpg",
    imageAlt: "Asfaltado en tramo de la Av. Municipal",
    status: "completed",
    progress: 100,
    startDate: "16/11/2025",
    endDate: "21/11/2025",
    budget: "200 Ton Asfalto",
    beneficiaries: "150 Metros lineales (Est.)",
    contractor: "Covimus",
    coordinates: { lat: 10.214109, lng: -64.632568 },
    impact: "Optimización de la movilidad urbana y seguridad vial.",
    testimonial: null
  },
  {
    id: 8,
    name: "Rehabilitación Av. Municipal (Isla de Cuba - Bomberos)",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Isla de Cuba",
    description: "Colocación de 350 toneladas de asfalto en tramo clave de la ciudad.",
    fullDescription: "Como parte vital del plan de recuperación vial del municipio, la Covimus ejecutó la rehabilitación del tramo de la Avenida Municipal comprendido entre la Isla de Cuba (prolongación de la Av. Alterna) y la Redoma de Los Bomberos. Con la colocación de 350 toneladas de asfalto, esta intervención consolida la movilidad en este eje estratégico. El gobernador Luis Marcano y el alcalde Jesús Marcano Tabata continúan trabajando articuladamente en la recuperación de calles y avenidas, sumando esfuerzos para transformar la infraestructura vial de toda la zona metropolitana.",
    images: [
      "/assets/images/ima_projects/2025/Isla_Cuba/av-municipal-isla-cuba-01.jpg",
      "/assets/images/ima_projects/2025/Isla_Cuba/av-municipal-isla-cuba-02.jpg",
      "/assets/images/ima_projects/2025/Isla_Cuba/av-municipal-isla-cuba-03.jpg",
      "/assets/images/ima_projects/2025/Isla_Cuba/av-municipal-isla-cuba-04.jpg",
      "/assets/images/ima_projects/2025/Isla_Cuba/av-municipal-isla-cuba-05.jpg",
      "/assets/images/ima_projects/2025/Isla_Cuba/av-municipal-isla-cuba-06.jpg"
    ],
    image: "/assets/images/ima_projects/2025/Isla_Cuba/av-municipal-isla-cuba-01.jpg",
    imageAlt: "Asfaltado tramo Isla de Cuba - Bomberos",
    status: "completed",
    progress: 100,
    startDate: "18/11/2025",
    endDate: "20/11/2025",
    budget: "727 Ton Asfalto",
    beneficiaries: "2,000 m²",
    contractor: "Covimus",
    coordinates: { lat: 10.195494, lng: -64.637249 },
    impact: "Consolidación de la movilidad en eje estratégico.",
    testimonial: null
  },
  {
    id: 9,
    name: "Rehabilitación Av. Universidad (Frente a antiguo Makro)",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Av. Universidad",
    description: "Colocación de 750 toneladas de asfalto mejorando la fluidez vehicular.",
    fullDescription: "Con un avance concreto para Puerto La Cruz, se ejecutó la colocación de 750 toneladas de mezcla asfáltica en caliente en la Avenida Universidad, específicamente frente al antiguo Makro. Esta importante obra de infraestructura, lograda gracias al trabajo articulado del Gobierno Nacional, Regional y Municipal, mejora significativamente la fluidez vehicular y la seguridad en esta arteria principal, beneficiando a miles de conductores diariamente.",
    images: [
      "/assets/images/ima_projects/2025/Avenida_Universidad/rehabilitacion-av-universidad-01.jpg",
      "/assets/images/ima_projects/2025/Avenida_Universidad/rehabilitacion-av-universidad-02.jpg",
      "/assets/images/ima_projects/2025/Avenida_Universidad/rehabilitacion-av-universidad-03.jpg",
      "/assets/images/ima_projects/2025/Avenida_Universidad/rehabilitacion-av-universidad-04.jpg",
      "/assets/images/ima_projects/2025/Avenida_Universidad/rehabilitacion-av-universidad-05.jpg",
      "/assets/images/ima_projects/2025/Avenida_Universidad/rehabilitacion-av-universidad-06.jpg",
      "/assets/images/ima_projects/2025/Avenida_Universidad/rehabilitacion-av-universidad-07.jpg"
    ],
    image: "/assets/images/ima_projects/2025/Avenida_Universidad/rehabilitacion-av-universidad-01.jpg",
    imageAlt: "Asfaltado en Av. Universidad frente a antiguo Makro",
    status: "completed",
    progress: 100,
    startDate: "08/11/2025",
    endDate: "12/11/2025",
    budget: "800 Ton Asfalto",
    beneficiaries: "2,000 m²",
    contractor: "Covimus",
    coordinates: { lat: 10.182618, lng: -64.646196 },
    impact: "Mejora significativa de la fluidez y seguridad vehicular.",
    testimonial: null
  },
  {
    id: 10,
    name: "Rehabilitación Calle El Comercio y Calle Colegio (Pozuelos)",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Pozuelos",
    description: "Recuperación vial para movilidad escolar y comercial en Pozuelos.",
    fullDescription: "El Gobierno Nacional, la Gobernación de Anzoátegui y la Alcaldía de Puerto La Cruz unieron esfuerzos para la rehabilitación integral de la Calle El Comercio y Calle Colegio en el sector Pozuelos. Esta obra es fundamental para la actividad económica local y facilita el acceso seguro de estudiantes y docentes a las instituciones educativas de la zona. La intervención mejora la movilidad, reduce tiempos de traslado y garantiza la seguridad de peatones y conductores, reafirmando que el bienestar de las comunidades es prioridad de la gestión revolucionaria.",
    images: [
      "/assets/images/ima_projects/2025/Calle_Colegio/rehabilitacion-calle-colegio-pozuelos-01.jpg",
      "/assets/images/ima_projects/2025/Calle_Colegio/rehabilitacion-calle-colegio-pozuelos-02.jpg",
      "/assets/images/ima_projects/2025/Calle_Colegio/rehabilitacion-calle-colegio-pozuelos-03.jpg",
      "/assets/images/ima_projects/2025/Calle_Colegio/rehabilitacion-calle-colegio-pozuelos-04.jpg",
      "/assets/images/ima_projects/2025/Calle_Colegio/rehabilitacion-calle-colegio-pozuelos-05.jpg",
      "/assets/images/ima_projects/2025/Calle_Colegio/rehabilitacion-calle-colegio-pozuelos-06.jpg"
    ],
    image: "/assets/images/ima_projects/2025/Calle_Colegio/rehabilitacion-calle-colegio-pozuelos-01.jpg",
    imageAlt: "Rehabilitación de vialidad en sector Pozuelos",
    status: "completed",
    progress: 100,
    startDate: "04/11/2025",
    endDate: "08/11/2025",
    budget: "100 Ton Asfalto (Est.)",
    beneficiaries: "Comunidad Educativa y Comercial",
    contractor: "Covimus",
    coordinates: { lat: 10.182017, lng: -64.634824 },
    impact: "Movilidad segura para escuelas y comercio local.",
    testimonial: null
  },
  {
    id: 11,
    name: "Rehabilitación Av. Bolívar de Puerto La Cruz",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Av. Bolívar",
    description: "Colocación de 482 toneladas de asfalto recuperando la vía con dignidad.",
    fullDescription: "Inspeccionamos el avance del programa de asfaltado en la Avenida Bolívar de Puerto La Cruz, una obra que va más allá de la mejora en la movilidad: devuelve la dignidad y la esperanza a la ciudad. Con la colocación de 482 toneladas de asfalto, logramos recuperar esta vía fundamental, fortaleciendo el sentido de pertenencia y reafirmando el compromiso de transformar la vida de nuestras comunidades. Gracias al respaldo del presidente Nicolás Maduro y la Gobernación de Anzoátegui, seguimos avanzando hacia una ciudad más segura, transitable y humana.",
    images: [
      "/assets/images/ima_projects/2025/Av.Bolivar/rehabilitacion-av-bolivar-01.jpg",
      "/assets/images/ima_projects/2025/Av.Bolivar/rehabilitacion-av-bolivar-02.jpg",
      "/assets/images/ima_projects/2025/Av.Bolivar/rehabilitacion-av-bolivar-03.jpg",
      "/assets/images/ima_projects/2025/Av.Bolivar/rehabilitacion-av-bolivar-04.jpg",
      "/assets/images/ima_projects/2025/Av.Bolivar/rehabilitacion-av-bolivar-05.jpg",
      "/assets/images/ima_projects/2025/Av.Bolivar/rehabilitacion-av-bolivar-06.jpg",
      "/assets/images/ima_projects/2025/Av.Bolivar/rehabilitacion-av-bolivar-07.jpg",
      "/assets/images/ima_projects/2025/Av.Bolivar/rehabilitacion-av-bolivar-08.jpg",
      "/assets/images/ima_projects/2025/Av.Bolivar/rehabilitacion-av-bolivar-09.jpg"
    ],
    image: "/assets/images/ima_projects/2025/Av.Bolivar/rehabilitacion-av-bolivar-01.jpg",
    imageAlt: "Asfaltado y recuperación de la Av. Bolívar",
    status: "completed",
    progress: 100,
    startDate: "24/08/2025",
    endDate: "28/08/2025",
    budget: "482 Ton Asfalto",
    beneficiaries: "Av. Bolívar Completa (Est.)",
    contractor: "Covimus",
    coordinates: { lat: 10.204096, lng: -64.636681 },
    impact: "Recuperación de arteria vial principal con dignidad y esfuerzo.",
    testimonial: "Tenía años esperando ver la Av. Bolívar así. El asfalto quedó como una mesa de billar. Da gusto transitar por aquí ahora.",
    testimonialAuthor: "Carmen Elena Velásquez",
    testimonialRole: "Conductora y Habitante"
  },
  {
    id: 12,
    name: "Avenida 5 de Julio",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Casco Central",
    description: "Colocación de 31 toneladas de asfalto en el corazón de la ciudad.",
    fullDescription: "Como parte del plan estratégico de recuperación del Casco Central, la Corporación de Vialidad e Infraestructura del Municipio Sotillo (COVIMUS), bajo los lineamientos del Alcalde Jesús Marcano Tábata, ejecutó una jornada de asfaltado en la importante Avenida 5 de Julio.\n\nLos trabajos consistieron en la colocación eficiente de 31 toneladas de mezcla asfáltica en caliente, permitiendo recuperar aproximadamente 45 metros lineales de vialidad. Esta obra optimiza significativamente el flujo vehicular en una de las arterias más transitadas de Puerto La Cruz, dinamizando la economía local y beneficiando directamente a la Comuna Bahía de Pozuelo, comerciantes y transportistas.",
    images: [
      "/assets/images/ima_projects/2026/Av.5_julio/av-5-julio-01.jpg",
      "/assets/images/ima_projects/2026/Av.5_julio/av-5-julio-02.jpg",
      "/assets/images/ima_projects/2026/Av.5_julio/av-5-julio-03.jpg",
      "/assets/images/ima_projects/2026/Av.5_julio/av-5-julio-04.jpg",
      "/assets/images/ima_projects/2026/Av.5_julio/av-5-julio-05.jpg",
      "/assets/images/ima_projects/2026/Av.5_julio/av-5-julio-06.jpg"
    ],
    image: "/assets/images/ima_projects/2026/Av.5_julio/av-5-julio-01.jpg",
    imageAlt: "Asfaltado en Av. 5 de Julio, Casco Central",
    status: "completed",
    progress: 100,
    startDate: "16/02/2026",
    endDate: "19/02/2026",
    budget: "31 Ton Asfalto",
    beneficiaries: "45 Metros",
    contractor: "Covimus",
    coordinates: { lat: 10.218213, lng: -64.637995 },
    impact: "Recuperación del Casco Central para dinamizar economía local.",
    testimonial: null
  },
  {
    id: 13,
    name: "Mejoras de Alcantarillado en Calle Montes",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Calle Montes",
    description: "Trabajos de saneamiento ambiental y mejoras del sistema de alcantarillado.",
    fullDescription: "Como parte del plan integral de saneamiento ambiental y seguridad vial que adelanta el municipio Sotillo, la Corporación de Vialidad e Infraestructura del Municipio Sotillo (Covimus) ejecutó trabajos de mejoras del sistema de alcantarillado en la calle Montes. La intervención garantiza un mejor drenaje pluvial y contribuye a la seguridad vial del sector, en articulación con la Secretaría de Ciudades Humanas. El alcalde Jesús Marcano Tabata, junto al equipo operativo, se mantiene supervisando el desarrollo de este tipo de obras en materia de servicios públicos, con el respaldo del gobernador Luis Marcano, dando cumplimiento al Plan de la Patria de las Siete Transformaciones (7T).",
    images: [
      "/assets/images/ima_projects/2026/Calle_Montes/alcantarillado-calle-montes-01.webp",
      "/assets/images/ima_projects/2026/Calle_Montes/alcantarillado-calle-montes-02.webp"
    ],
    image: "/assets/images/ima_projects/2026/Calle_Montes/alcantarillado-calle-montes-01.webp",
    imageAlt: "Mejoras del sistema de alcantarillado en Calle Montes",
    status: "completed",
    progress: 100,
    startDate: "08/04/2026",
    endDate: "13/04/2026",
    budget: "Alcantarillado y Saneamiento",
    beneficiaries: "300 Metros lineales (Est.)",
    contractor: "Covimus",
    coordinates: { lat: 10.199820, lng: -64.635185 },
    impact: "Mejora del drenaje pluvial y saneamiento ambiental del sector.",
    testimonial: "El municipio está en buenas manos del alcalde Jesús Marcano, eficiente al momento de trabajar.",
    testimonialAuthor: "giovannarojasf",
    testimonialRole: "Seguidor en redes sociales"
  },
  {
    id: 14,
    name: "Demarcación Vial y Tachas Reflectivas – Av. Municipal (Tramo UDO)",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Av. Municipal",
    description: "Demarcación vial y colocación de tachas reflectivas en el tramo del semáforo de la UDO.",
    fullDescription: "A través de la Secretaría de Ciudades Humanas, la Corporación de Vialidad e Infraestructura del Municipio Sotillo (Covimus) completó los trabajos de demarcación y señalización vial en la Avenida Municipal, en el tramo que va desde el semáforo de la Universidad de Oriente (UDO), garantizando un tránsito más ordenado para conductores y estudiantes. La intervención se enmarca en el plan integral que ya suma 135,96 km de vías demarcadas y la instalación de 12.100 tachas reflectivas (ojos de gato) en todo el municipio Sotillo, además de la adecuación de 1.610 metros de pasos peatonales. El alcalde Jesús Marcano Tabata destacó que estos trabajos van acompañados de seguridad vial y saneamiento ambiental, en articulación con el gobernador Luis Marcano y el respaldo de la Presidenta (E) Delcy Rodríguez, dando cumplimiento al Plan de la Patria de las Siete Transformaciones (7T).",
    images: [
      "/assets/images/ima_projects/2026/Av_Municipal_UDO/demarcacion-av-municipal-udo-01.webp",
      "/assets/images/ima_projects/2026/Av_Municipal_UDO/demarcacion-av-municipal-udo-02.webp",
      "/assets/images/ima_projects/2026/Av_Municipal_UDO/demarcacion-av-municipal-udo-03.webp"
    ],
    image: "/assets/images/ima_projects/2026/Av_Municipal_UDO/demarcacion-av-municipal-udo-01.webp",
    imageAlt: "Demarcación vial y tachas reflectivas en Av. Municipal, tramo UDO",
    status: "completed",
    progress: 100,
    startDate: "08/04/2026",
    endDate: "13/04/2026",
    budget: "Demarcación y Tachas Reflectivas",
    beneficiaries: "300 Metros lineales (Est.)",
    contractor: "Covimus",
    coordinates: { lat: 10.187911, lng: -64.640779 },
    impact: "Tránsito más ordenado y seguro para conductores y estudiantes de la zona UDO.",
    testimonial: "Excelente el trabajo que se está realizando, todo requiere de dedicación y tiempo y es lo que realmente se está notando en el municipio y eso es lo importante.",
    testimonialAuthor: "fabelcristinasalazar",
    testimonialRole: "Seguidor en redes sociales"
  },
  {
    id: 15,
    name: "Plan Nacional \"Resuelve tu Calle\" — Av. 5 de Julio",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Casco Central",
    description: "Aplicación de asfalto en frío del Plan Nacional \"Resuelve tu Calle\" en el Casco Central.",
    fullDescription: "En el marco de la Segunda Transformación del Plan de la Patria, la Corporación de Vialidad e Infraestructura del Municipio Sotillo (Covimus), junto al alcalde Jesús Marcano Tabata, la comunidad y transportistas, desplegó el Plan Nacional de Asfaltado \"Resuelve tu Calle\" en la Avenida 5 de Julio, en pleno Casco Central de Puerto La Cruz. Esta alternativa de asfaltado en frío ya había sido utilizada en el bacheo y recuperación de la Avenida Municipal, a la altura de la Redoma del Heptaedro al Cielo, y ahora se extiende al casco central de la ciudad, con miras a trasladarse próximamente a las comunidades, en articulación con el gobernador Luis Marcano, para empoderar a las comunas en la rehabilitación de las calles de sus sectores. El plan es una iniciativa del Gobierno Nacional a través del Ministerio del Poder Popular de Obras Públicas, que utiliza asfalto de calidad extraído del Lago de Guanoco, en el estado Sucre, con el respaldo de la Presidenta (E) Delcy Rodríguez.",
    images: [
      "/assets/images/ima_projects/2026/ResuelveTuCalle_Av5Julio/resuelve-tu-calle-av-5-julio-01.webp",
      "/assets/images/ima_projects/2026/ResuelveTuCalle_Av5Julio/resuelve-tu-calle-av-5-julio-02.webp",
      "/assets/images/ima_projects/2026/ResuelveTuCalle_Av5Julio/resuelve-tu-calle-av-5-julio-03.webp",
      "/assets/images/ima_projects/2026/ResuelveTuCalle_Av5Julio/resuelve-tu-calle-av-5-julio-04.webp",
      "/assets/images/ima_projects/2026/ResuelveTuCalle_Av5Julio/resuelve-tu-calle-av-5-julio-05.webp"
    ],
    image: "/assets/images/ima_projects/2026/ResuelveTuCalle_Av5Julio/resuelve-tu-calle-av-5-julio-01.webp",
    imageAlt: "Plan Nacional de Asfaltado \"Resuelve tu Calle\" en la Av. 5 de Julio, Casco Central",
    status: "completed",
    progress: 100,
    startDate: "17/04/2026",
    endDate: "21/04/2026",
    budget: "Asfalto en Frío (Resuelve tu Calle)",
    beneficiaries: "Sin dato disponible",
    contractor: "Covimus",
    coordinates: { lat: 10.221266, lng: -64.634353 },
    impact: "Recuperación vial en el Casco Central mediante asfaltado en frío de calidad.",
    testimonial: null,
    testimonialAuthor: null,
    testimonialRole: null
  },
  {
    id: 16,
    name: "Plan \"Resuelve Tu Calle\" — Inicio Av. 5 de Julio",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Casco Central",
    description: "Bacheo con asfalto frío para la rehabilitación de rutas de transporte público en el inicio de la Av. 5 de Julio.",
    fullDescription: "La Autoridad de la Corporación de Vialidad e Infraestructura del Municipio Sotillo (Covimus), Oswaldo González, junto al director de movilidad y transporte, Rubén Figueroa, y un grupo de transportistas, participaron en la continuidad del Plan Nacional \"Resuelve Tu Calle\" en el inicio de la Avenida 5 de Julio. Este programa piloto, impulsado por el Gobierno Nacional, tiene como objetivo el bacheo para la rehabilitación de las rutas del transporte público utilizando asfalto en frío. La colaboración de las comunidades y los transportistas resulta fundamental para el éxito de este proyecto, que continúa transformando la red vial del municipio Sotillo.",
    images: [
      "/assets/images/ima_projects/2026/ResuelveTuCalle_InicioAv5Julio/resuelve-tu-calle-inicio-av-5-julio-01.webp",
      "/assets/images/ima_projects/2026/ResuelveTuCalle_InicioAv5Julio/resuelve-tu-calle-inicio-av-5-julio-02.webp",
      "/assets/images/ima_projects/2026/ResuelveTuCalle_InicioAv5Julio/resuelve-tu-calle-inicio-av-5-julio-03.webp",
      "/assets/images/ima_projects/2026/ResuelveTuCalle_InicioAv5Julio/resuelve-tu-calle-inicio-av-5-julio-04.webp",
      "/assets/images/ima_projects/2026/ResuelveTuCalle_InicioAv5Julio/resuelve-tu-calle-inicio-av-5-julio-05.webp"
    ],
    image: "/assets/images/ima_projects/2026/ResuelveTuCalle_InicioAv5Julio/resuelve-tu-calle-inicio-av-5-julio-01.webp",
    imageAlt: "Bacheo con asfalto frío en el inicio de la Av. 5 de Julio — Plan Resuelve Tu Calle",
    status: "completed",
    progress: 100,
    startDate: "14/05/2026",
    endDate: "18/05/2026",
    budget: "Asfalto en Frío (Bacheo)",
    beneficiaries: "Sin dato disponible",
    contractor: "Covimus",
    coordinates: { lat: 10.226931, lng: -64.628415 },
    impact: "Rehabilitación de rutas de transporte público mediante bacheo con asfalto en frío.",
    testimonial: null,
    testimonialAuthor: null,
    testimonialRole: null
  },
  {
    id: 17,
    name: "Bacheo con Asfalto Frío en Calle El Chimborazo (Bahía de Pozuelos)",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Calle El Chimborazo",
    description: "Bacheo con asfalto frío en la calle El Chimborazo, comuna Bahía de Pozuelos.",
    fullDescription: "Continúa la recuperación de calles en el municipio Sotillo. La Corporación de Vialidad e Infraestructura del Municipio Sotillo (Covimus) mantiene activos sus planes de rehabilitación urbana, desplegando cuadrillas operativas para labores de bacheo con asfalto frío en la calle El Chimborazo, perteneciente a la comuna Bahía de Pozuelos en Revolución. Estos trabajos son impulsados por la gestión del alcalde Jesús Marcano Tabata, en un esfuerzo conjunto con las 21 comunas de la jurisdicción, cumpliendo con el Plan de las Siete Transformaciones (7T) del gobierno nacional y regional en materia de servicios públicos, para dar respuestas directas a las comunidades y optimizar la vialidad de Puerto La Cruz.",
    images: [
      "/assets/images/ima_projects/2026/Calle_Chimborazo/calle-chimborazo-pozuelos-01.webp",
      "/assets/images/ima_projects/2026/Calle_Chimborazo/calle-chimborazo-pozuelos-02.webp",
      "/assets/images/ima_projects/2026/Calle_Chimborazo/calle-chimborazo-pozuelos-03.webp",
      "/assets/images/ima_projects/2026/Calle_Chimborazo/calle-chimborazo-pozuelos-04.webp"
    ],
    image: "/assets/images/ima_projects/2026/Calle_Chimborazo/calle-chimborazo-pozuelos-01.webp",
    imageAlt: "Bacheo con asfalto frío en la calle El Chimborazo, comuna Bahía de Pozuelos",
    status: "completed",
    progress: 100,
    startDate: "24/06/2026",
    endDate: "24/06/2026",
    budget: "Asfalto en Frío (Bacheo)",
    beneficiaries: "Sin dato disponible",
    contractor: "Covimus",
    coordinates: { lat: 10.200791, lng: -64.637879 },
    impact: "Continuidad de la rehabilitación vial en la comuna Bahía de Pozuelos.",
    testimonial: null
  },
  {
    id: 18,
    name: "Bacheo en Calle Andrés Eloy Blanco con Calle Democracia (Sierra Maestra)",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Sierra Maestra",
    description: "Bacheo con asfalto frío en la calle Andrés Eloy Blanco con calle Democracia, sector Sierra Maestra.",
    fullDescription: "Se mantienen los trabajos de rehabilitación vial en calles del municipio Sotillo a través del Plan 1x10 del Buen Gobierno. En respuesta a la solicitud de la comunidad de Sierra Maestra en materia de rehabilitación vial, la Corporación de Vialidad e Infraestructura del municipio Sotillo (Covimus) ejecutó labores de bacheo en la calle Andrés Eloy Blanco con calle Democracia, ubicadas en el referido sector de la ciudad. El despliegue técnico consistió en la colocación de 60 sacos de asfalto frío, con el propósito de optimizar la circulación vehicular y peatonal. Con estos trabajos, la alcaldía del municipio Sotillo liderada por el alcalde Jesús Marcano Tabata reafirma su compromiso con el mantenimiento de la infraestructura urbana, priorizando la recuperación de las vías públicas y consolidando el bienestar de todos los sectores, en el marco del Plan de las Siete Transformaciones (7T), impulsado por el gobierno nacional, regional y municipal.",
    images: [
      "/assets/images/ima_projects/2026/Sierra_Maestra/eloy-blanco-democracia-sierra-maestra-01.webp",
      "/assets/images/ima_projects/2026/Sierra_Maestra/eloy-blanco-democracia-sierra-maestra-02.webp",
      "/assets/images/ima_projects/2026/Sierra_Maestra/eloy-blanco-democracia-sierra-maestra-03.webp",
      "/assets/images/ima_projects/2026/Sierra_Maestra/eloy-blanco-democracia-sierra-maestra-04.webp",
      "/assets/images/ima_projects/2026/Sierra_Maestra/eloy-blanco-democracia-sierra-maestra-05.webp"
    ],
    image: "/assets/images/ima_projects/2026/Sierra_Maestra/eloy-blanco-democracia-sierra-maestra-01.webp",
    imageAlt: "Bacheo con asfalto frío en la calle Andrés Eloy Blanco con calle Democracia, sector Sierra Maestra",
    status: "completed",
    progress: 100,
    startDate: "02/07/2026",
    endDate: "02/07/2026",
    budget: "60 Sacos Asfalto Frío",
    beneficiaries: "Sin dato disponible",
    contractor: "Covimus",
    coordinates: { lat: 10.195772, lng: -64.639525 },
    impact: "Optimización de la circulación vehicular y peatonal en Sierra Maestra.",
    testimonial: null
  },
  {
    id: 19,
    name: "Bacheo con Asfalto Frío en el Sector Bella Vista",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Bella Vista",
    description: "Bacheo con asfalto frío en el sector Bella Vista mediante la colocación de 80 sacos.",
    fullDescription: "Continúan los trabajos de bacheo en el municipio Sotillo. Cuadrillas operativas de la Corporación de Vialidad e Infraestructura del municipio Sotillo (Covimus) continúan desplegadas en la jurisdicción ejecutando labores de mantenimiento vial. En esta oportunidad, se dio continuidad a los trabajos de bacheo en el sector Bella Vista con la colocación de 80 sacos de asfalto frío. Esta acción forma parte de la respuesta inmediata de la gestión municipal del alcalde Jesús Marcano Tabata a los reportes recibidos a través de la plataforma del 1x10 del Buen Gobierno, herramienta fundamental diseñada por el gobierno nacional para atender las necesidades prioritarias de las comunidades.",
    images: [
      "/assets/images/ima_projects/2026/Bella_Vista/bacheo-bella-vista-01.webp",
      "/assets/images/ima_projects/2026/Bella_Vista/bacheo-bella-vista-02.webp",
      "/assets/images/ima_projects/2026/Bella_Vista/bacheo-bella-vista-03.webp",
      "/assets/images/ima_projects/2026/Bella_Vista/bacheo-bella-vista-04.webp",
      "/assets/images/ima_projects/2026/Bella_Vista/bacheo-bella-vista-05.webp"
    ],
    image: "/assets/images/ima_projects/2026/Bella_Vista/bacheo-bella-vista-01.webp",
    imageAlt: "Bacheo con asfalto frío en el sector Bella Vista",
    status: "completed",
    progress: 100,
    startDate: "07/07/2026",
    endDate: "07/07/2026",
    budget: "80 Sacos Asfalto Frío",
    beneficiaries: "Sin dato disponible",
    contractor: "Covimus",
    coordinates: { lat: 10.204485, lng: -64.635936 },
    impact: "Respuesta inmediata a los reportes de la comunidad a través del 1x10 del Buen Gobierno.",
    testimonial: null
  },
  {
    id: 20,
    name: "Bacheo con Asfalto Frío en Av. Intercomunal (Sector Cerro Amarillo)",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "Cerro Amarillo",
    description: "Bacheo con asfalto frío en la Av. Intercomunal, sector Cerro Amarillo, con la colocación de 20 sacos.",
    fullDescription: "Seguimos desplegados en la Av. Intercomunal. Nuestras cuadrillas de la Corporación de Vialidad e Infraestructura del municipio Sotillo (Covimus) se mantienen desplegadas en cada rincón de la jurisdicción, garantizando vías óptimas y seguras para la gente. En esta oportunidad, se ejecutaron labores de mantenimiento vial en la Av. Intercomunal, específicamente en el sector Cerro Amarillo (Comuna General José Antonio Anzoátegui 25, Eje 6), con la colocación de 20 sacos de asfalto frío. Esta acción forma parte de la respuesta inmediata de la gestión municipal del alcalde Jesús Marcano Tabata a los reportes recibidos a través de la plataforma 1x10 del Buen Gobierno, herramienta fundamental diseñada por el gobierno nacional para priorizar el bienestar de las comunidades.",
    images: [
      "/assets/images/ima_projects/2026/Cerro_Amarillo/bacheo-intercomunal-cerro-amarillo-01.webp",
      "/assets/images/ima_projects/2026/Cerro_Amarillo/bacheo-intercomunal-cerro-amarillo-02.webp",
      "/assets/images/ima_projects/2026/Cerro_Amarillo/bacheo-intercomunal-cerro-amarillo-03.webp"
    ],
    image: "/assets/images/ima_projects/2026/Cerro_Amarillo/bacheo-intercomunal-cerro-amarillo-01.webp",
    imageAlt: "Bacheo con asfalto frío en la Av. Intercomunal, sector Cerro Amarillo",
    status: "completed",
    progress: 100,
    startDate: "15/07/2026",
    endDate: "15/07/2026",
    budget: "20 Sacos Asfalto Frío",
    beneficiaries: "Sin dato disponible",
    contractor: "Covimus",
    coordinates: { lat: 10.183459, lng: -64.646662 },
    impact: "Respuesta inmediata a los reportes de la comunidad a través del 1x10 del Buen Gobierno.",
    testimonial: null
  },
  {
    id: 21,
    name: "Asfalto Frío en el Elevado de Puerto La Cruz (Eje 5)",
    category: "infrastructure",
    categoryIcon: "Construction",
    municipality: "El Elevado",
    description: "Aplicación de asfalto en frío en el Elevado de Puerto La Cruz con la colocación de 5 sacos.",
    fullDescription: "La gerencia de operaciones de la Corporación de Vialidad e Infraestructura del municipio Sotillo (Covimus) ejecutó con éxito la aplicación de asfalto en frío en el Elevado de Puerto La Cruz, Comuna Bahía de Pozuelos en Revolución (Eje 5). Con la colocación de 5 sacos de asfalto frío, se garantiza la seguridad vial de los usuarios que diariamente transitan por esta importante estructura vial. Todo esto es posible gracias al apoyo y compromiso del alcalde Jesús Marcano Tabata, quien sigue impulsando el desarrollo y el bienestar del municipio.",
    images: [
      "/assets/images/ima_projects/2026/Elevado_PLC/asfalto-frio-elevado-plc-01.webp",
      "/assets/images/ima_projects/2026/Elevado_PLC/asfalto-frio-elevado-plc-02.webp",
      "/assets/images/ima_projects/2026/Elevado_PLC/asfalto-frio-elevado-plc-03.webp",
      "/assets/images/ima_projects/2026/Elevado_PLC/asfalto-frio-elevado-plc-04.webp",
      "/assets/images/ima_projects/2026/Elevado_PLC/asfalto-frio-elevado-plc-05.webp"
    ],
    image: "/assets/images/ima_projects/2026/Elevado_PLC/asfalto-frio-elevado-plc-01.webp",
    imageAlt: "Aplicación de asfalto en frío en el Elevado de Puerto La Cruz",
    status: "completed",
    progress: 100,
    startDate: "17/07/2026",
    endDate: "17/07/2026",
    budget: "5 Sacos Asfalto Frío",
    beneficiaries: "Sin dato disponible",
    contractor: "Covimus",
    coordinates: { lat: 10.210692, lng: -64.633363 },
    impact: "Seguridad vial garantizada en una estructura clave de Puerto La Cruz.",
    testimonial: null
  }
];